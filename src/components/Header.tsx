  ...
  return (
    <header 
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${
        scrolled ? 'bg-raelan-bg/90 backdrop-blur-md py-3 shadow-md' : 'bg-transparent py-6'
      }`}
    >
      <div className="container mx-auto flex justify-between items-center px-4 md:px-6">
        ...
        {/* Mobile Menu Button */}
        <button 
          className="md:hidden text-raelan-offwhite hover:text-raelan-gold"
      ...
      <div className={`md:hidden ${mobileMenuOpen ? 'block' : 'hidden'}`}>
        <div className="fixed inset-0 bg-raelan-bg/95 z-40 flex flex-col items-center justify-center">
          <button 
            className="absolute top-6 right-6 text-raelan-offwhite hover:text-raelan-gold"
            onClick={toggleMobileMenu}
